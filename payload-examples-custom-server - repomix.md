This file is a merged representation of `payload/examples/custom-server/` codebase (https://github.com/payloadcms/payload/tree/main/examples/custom-server), combined into a single document by Repomix. Next.js includes its own server with `next start` by default. If you have an existing backend, you can still use it with Next.js (this is not a custom server). A custom Next.js server allows you to programmatically start a server for custom patterns. The majority of the time, you will not need this approach. However, it's available if you need to eject.
