...
### Testing development / unstable builds[​](#testing-development--unstable-builds "Direct link to Testing development / unstable builds")
...
### Building your own images[​](#building-your-own-images "Direct link to Building your own images")
...
### Security note[​](#security-note "Direct link to Security note")
...
### External Ports[​](#external-ports "Direct link to External Ports")
...
IP addresses of the Docker host (comma separated), needed for LAN environments
...
#### Let's Encrypt configuration[​](#lets-encrypt-configuration "Direct link to Let's Encrypt configuration")
...
#### Using existing TLS certificate and key[​](#using-existing-tls-certificate-and-key "Direct link to Using existing TLS certificate and key")
...
### Features configuration (config.js)[​](#features-configuration-configjs "Direct link to Features configuration (config.js)")
...
### Jigasi SIP gateway (audio only) configuration[​](#jigasi-sip-gateway-audio-only-configuration "Direct link to Jigasi SIP gateway (audio only) configuration")
...
[test@sip2sip.info](mailto:test@sip2sip.info)
...
#### Display Dial-In information[​](#display-dial-in-information "Direct link to Display Dial-In information")
...
URL to the JSON with all Dial-In numbers
...
### Recording / live streaming configuration with Jibri[​](#recording--live-streaming-configuration-with-jibri "Direct link to Recording / live streaming configuration with Jibri")
...
Internal recorder user for Jibri client connections

recorder

`JIBRI_RECORDER_PASSWORD`

Internal recorder password for Jibri client connections
...
Internal user for Jibri client connections.

jibri

`JIBRI_STRIP_DOMAIN_JID`

Prefix domain for strip inside Jibri (please see env.example for details)
...
### Jitsi Meet configuration[​](#jitsi-meet-configuration "Direct link to Jitsi Meet configuration")
...
Select session timeout value for an authenticated user

3 hours

#### Internal authentication[​](#internal-authentication "Direct link to Internal authentication")
...
#### Authentication using LDAP[​](#authentication-using-ldap "Direct link to Authentication using LDAP")
...
CN=binduser,OU=users,DC=example,DC=domain,DC=com
...
Set TLS ciphers list to allow

SECURE256:SECURE128

`LDAP_TLS_CHECK_PEER`

Require and verify LDAP server certificate

1

`LDAP_TLS_CACERT_FILE`

Path to CA cert file. Used when server certificate verification is enabled

/etc/ssl/certs/ca-certificates.crt

`LDAP_TLS_CACERT_DIR`

Path to CA certs directory. Used when server certificate verification is enabled.

/etc/ssl/certs

`LDAP_START_TLS`

Enable START\_TLS, requires LDAPv3, URL must be ldap:// not ldaps://

0

#### Authentication using JWT tokens[​](#authentication-using-jwt-tokens "Direct link to Authentication using JWT tokens")
...
my\_jitsi\_app\_secret

`JWT_ACCEPTED_ISSUERS`

(Optional) Set asap\_accepted\_issuers as a comma separated list

my\_web\_client,my\_app\_client

`JWT_ACCEPTED_AUDIENCES`

(Optional) Set asap\_accepted\_audiences as a comma separated list

my\_server1,my\_server2

`JWT_ASAP_KEYSERVER`

(Optional) Set asap\_keyserver to a url where public keys can be found

[https://example.com/asap>](https://example.com/asap%3E)
...
(Optional) Controls which module is used for processing incoming JWTs

token

`JWT_TOKEN_AUTH_MODULE`

(Optional) Controls which module is used for validating JWTs
...
#### Authentication using Matrix[​](#authentication-using-matrix "Direct link to Authentication using Matrix")
...
`https://uvs.example.com:3000`

`MATRIX_UVS_ISSUER`

(optional) The issuer of the auth token to be passed through. Must match what is being set as `iss` in the JWT.
...
#### Authentication using Hybrid Matrix Token[​](#authentication-using-hybrid-matrix-token "Direct link to Authentication using Hybrid Matrix Token")
...
(optional) The issuer of the auth token to be passed through. Must match what is being set as `iss` in the JWT. It allows all issuers (`*`) by default.
...
(optional) Allow Matrix room members to bypass Jitsi lobby check.
...
#### External authentication[​](#external-authentication "Direct link to External authentication")
...
### Shared document editing using Etherpad[​](#shared-document-editing-using-etherpad "Direct link to Shared document editing using Etherpad")
...
### Transcription configuration[​](#transcription-configuration "Direct link to Transcription configuration")
...
Enable Jigasi transcription in a conference
...
`project_id` from Google Cloud Credentials

`GC_PRIVATE_KEY_ID`

`private_key_id` from Google Cloud Credentials

`GC_PRIVATE_KEY`

`private_key` from Google Cloud Credentials

`GC_CLIENT_EMAIL`

`client_email` from Google Cloud Credentials

`GC_CLIENT_ID`

`client_id` from Google Cloud Credentials

`GC_CLIENT_CERT_URL`

`client_x509_cert_url` from Google Cloud Credentials

`JIGASI_TRANSCRIBER_ADVERTISE_URL`

Jigasi will post an url to the chat with transcription file

true

`JIGASI_TRANSCRIBER_CUSTOM_SERVICE`

Jigasi will use this class for custom transcriptions instead of google cloud

`JIGASI_TRANSCRIBER_CUSTOM_TRANSLATION_SERVICE`

Jigasi will use this class for custom transctions instead of google cloud

`JIGASI_TRANSCRIBER_ENABLE_SAVING`

Jigasi will save results to a transcription file

true

`JIGASI_TRANSCRIBER_FILTER_SILENCE`

Jigasi will filter silent audio and not forward to backends

`JIGASI_TRANSCRIBER_LIBRETRANSLATE_URL`

URL for libretranslate services

`JIGASI_TRANSCRIBER_OCI_COMPARTMENT`

OCI compartment for use with Oracle Cloud Speech AI services

`JIGASI_TRANSCRIBER_OCI_REGION`

OCI region name for use with Oracle Cloud Speech AI services

`JIGASI_TRANSCRIBER_RECORD_AUDIO`

Jigasi will record audio when transcriber is on

true

`JIGASI_TRANSCRIBER_REMOTE_CONFIG_URL`

URL to control transcriber custom service based on conference details

`JIGASI_TRANSCRIBER_SEND_TXT`

Jigasi will send transcribed text to the chat when transcriber is on
...
URL for use with vosk backend

`JIGASI_TRANSCRIBER_WHISPER_URL`

URL for use with whisper backend

`JIGASI_TRANSCRIBER_WHISPER_PRIVATE_KEY_NAME`

Private Key ID of the private key to use with whisper

`JIGASI_TRANSCRIBER_WHISPER_PRIVATE_KEY`

Private Key material to use with whisper, without newlines or START/END delimiters

For setting the Google Cloud Credentials please read [https://cloud.google.com/text-to-speech/docs/quickstart-protocol](https://cloud.google.com/text-to-speech/docs/quickstart-protocol) > section "Before you begin" paragraph 1 to 5.

### Sentry logging configuration[​](#sentry-logging-configuration "Direct link to Sentry logging configuration")
...
### TURN server configuration[​](#turn-server-configuration "Direct link to TURN server configuration")
...
TURN server hostnames as a comma separated list (UDP or TCP transport)

`TURN_PORT`

TURN server port (UDP or TCP transport)

443

`TURN_TRANSPORT`

TURN server protocols as a comma separated list (UDP or TCP or both)

tcp

`TURNS_HOST`

TURN server hostnames as a comma separated list (TLS transport)

`TURNS_PORT`

TURN server port (TLS transport)

443

`TURN_TLL`

TURN max allocation duration (sec)
...
Internal XMPP server name xmpp.meet.jitsi

xmpp.meet.jitsi

`XMPP_BOSH_URL_BASE`

Internal XMPP server URL for BOSH module
...
Custom Prosody modules for XMPP\_DOMAIN (comma separated)
...
Custom prosody modules to load in global configuration (comma separated)

statistics,alert

`GLOBAL_CONFIG`

Custom configuration string with escaped newlines
...
Handle TLS connections outside of this setup
...
Send a `strict-transport-security` header to force browsers to use a secure and trusted connection. Recommended for production use.
...
Enabled older unsafe regex for JVB colibri-ws URLs. WARNING: Enable with caution, this regex allows connections to arbitrary internal IP addresses and is not recommended for production use. Unsafe regex is defined as `[a-zA-Z0-9-\._]+`

0

`COLIBRI_WEBSOCKET_JVB_LOOKUP_NAME`

DNS name to look up JVB IP address, used for default value of `COLIBRI_WEBSOCKET_REGEX`

jvb

`COLIBRI_WEBSOCKET_REGEX`

Overrides the colibri regex used for proxying to JVB. Recommended to override in production with values matching possible JVB IP ranges

defaults to `dig $COLIBRI_WEBSOCKET_JVB_LOOKUP_NAME` unless `DISABLE_COLIBRI_WEBSOCKET_JVB_LOOKUP` is set to true

`DISABLE_COLIBRI_WEBSOCKET_JVB_LOOKUP`

Controls whether to run `dig $COLIBRI_WEBSOCKET_JVB_LOOKUP_NAME` when defining COLIBRI\_WEBSOCKET\_REGEX

0

#### Advanced Prosody options[​](#advanced-prosody-options "Direct link to Advanced Prosody options")
...
Enable Prosody's reservation REST API

false

`PROSODY_RESERVATION_REST_BASE_URL`

Base URL of Prosody's reservation REST API

`PROSODY_AUTH_TYPE`

Select authentication type for Prosody (internal, jwt or ldap)

`AUTH_TYPE`

`PROSODY_ENABLE_METRICS`

Enables the http\_openmetrics module which exposes Prometheus metrics at `/metrics`

false

`PROSODY_METRICS_ALLOWED_CIDR`

CIDR block permitted to access metrics

172.16.0.0/12

#### Advanced Jicofo options[​](#advanced-jicofo-options "Direct link to Advanced Jicofo options")
...
Select authentication type for Jicofo (internal, jwt or ldap)
...
Enable health checks inside Jicofo, allowing the use of the REST api to check Jicofo's status

false

#### Advanced JVB options[​](#advanced-jvb-options "Direct link to Advanced JVB options")
...
UDP port for media used by Jitsi Videobridge

10000

`JVB_COLIBRI_PORT`

COLIBRI REST API port of JVB exposed to localhost
...
Enable the COLIBRI REST API

true

`SHUTDOWN_REST_ENABLED`

Enable the shutdown REST API

true

#### Advanced Jigasi options[​](#advanced-jigasi-options "Direct link to Advanced Jigasi options")
...
### Running behind NAT or on a LAN environment[​](#running-behind-nat-or-on-a-lan-environment "Direct link to Running behind NAT or on a LAN environment")
...
#### Split horizon[​](#split-horizon "Direct link to Split horizon")
...
#### Offline / airgapped installation[​](#offline--airgapped-installation "Direct link to Offline / airgapped installation")
...
### Adjust UDP buffers[​](#adjust-udp-buffers "Direct link to Adjust UDP buffers")
...
Accessing server logs[​](#accessing-server-logs "Direct link to Accessing server logs")
---------------------------------------------------------------------------------------
...
Build Instructions[​](#build-instructions "Direct link to Build Instructions")
------------------------------------------------------------------------------
...
Running behind a reverse proxy[​](#running-behind-a-reverse-proxy "Direct link to Running behind a reverse proxy")
------------------------------------------------------------------------------------------------------------------
...
### Disable HTTPS[​](#disable-https "Direct link to Disable HTTPS")

HTTPS can be disabled in the Docker Compose configuration (since HTTPS will probably not work on localhost):

    DISABLE_HTTPS=1ENABLE_HTTP_REDIRECT=0ENABLE_LETS_ENCRYPT=0

### Do not expose the Jitsi Meet's ports publicly[​](#do-not-expose-the-jitsi-meets-ports-publicly "Direct link to Do not expose the Jitsi Meet's ports publicly")
...
By default, the `HTTP_PORT` and `HTTPS_PORT` are binding to any ip address, so are publicly open unless a firewall blocks them. When using a reverse proxy, this is not necessary. This can be changed by updating the web container's ports configuration:
                ...
                - '127.0.0.1:${HTTP_PORT}:80'            - '127.0.0.1:${HTTPS_PORT}:443'

instead of

                - '${HTTP_PORT}:80'            - '${HTTPS_PORT}:443'

### Reverse proxy configuration[​](#reverse-proxy-configuration "Direct link to Reverse proxy configuration")
...
With nginx, these routes can be forwarded using the following config snippet:
    ...
    location /xmpp-websocket {    proxy_pass http://localhost:8000/xmpp-websocket;    proxy_http_version 1.1;    proxy_set_header Upgrade $http_upgrade;    proxy_set_header Connection "upgrade";}location /colibri-ws {    proxy_pass http://localhost:8000/colibri-ws;    proxy_http_version 1.1;    proxy_set_header Upgrade $http_upgrade;    proxy_set_header Connection "upgrade";}location / {    proxy_pass http://localhost:8000/;    proxy_http_version 1.1;}
...
The reverse proxy can be configured using the following config snippet:
    ...
    <IfModule mod_proxy.c>    <IfModule mod_proxy_wstunnel.c>        ProxyTimeout 900        ProxyPass /xmpp-websocket ws://localhost:8000/xmpp-websocket        ProxyPass /colibri-ws/ ws://localhost:8000/colibri-ws/        ProxyPass / http://localhost:8000/        ProxyPassReverse / http://localhost:8000/    </IfModule></IfModule>

### Disabling WebSocket connections[​](#disabling-websocket-connections "Direct link to Disabling WebSocket connections")
